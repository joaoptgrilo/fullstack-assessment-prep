...
// Type definitions for our API data shapes
interface Poll {
  id: number;
  question: string;
}
...
async function startServer() {
  try {
    ...
    app.get("/api/v1/polls/:id", (req: Request, res: Response) => {
      ...
      const optionsSql = "SELECT id, option_text, votes FROM options WHERE poll_id = ?";

      db.get(pollSql, [req.params.id], (err: Error | null, poll: Poll) => {
        if (err) return res.status(500).json({ error: err.message });
        if (!poll) return res.status(404).json({ error: "Poll not found." });

        db.all(optionsSql, [req.params.id], (err: Error | null, options: Option[]) => {
          if (err) return res.status(500).json({ error: err.message });
          res.status(200).json({ id: poll.id, question: poll.question, options });
        }
        );
      });
    });
